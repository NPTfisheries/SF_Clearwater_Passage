...
This report presents a preliminary assessment of adult steelhead passage at a suspected velocity barrier on the South Fork Clearwater River using detections of PIT-tagged fish. Key detection sites include SC3 (downstream of the barrier) and SC4 (upstream). The analysis includes adults PIT-tagged at Lower Granite Dam and juveniles tagged at various locations, including within the South Fork Clearwater River. 

We focus on whether PIT-tagged adults detected at SC3 later appear at SC4, indicating successful upstream passage — or a lack thereof — across the high-velocity reach. We also examine how factors such as juvenile release location and river discharge relate to "conversion" rates. A conversion rate indicates the proportion of adult salmon that successfully transition between key points during upstream migration and serves as a proxy for both survival and passage success. Although the primary focus is steelhead, adult spring/summer Chinook salmon PIT detections are included for comparison.
...
Conversion rates from SC3 to SC4 were above 85% for sp/sum Chinook salmon; however, conversion rates for steelhead were all below 11% (Figure \@ref(fig:conv-rate-p)).
...
Estimates of conversion rates can be confounded by a variety of biological and logistical reasons. For example, most sp/sum Chinook salmon in the South Fork Clearwater River originate from natal tributaries located upstream of the high-velocity reach. In contrast, many steelhead originate from hatchery releases downstream of this reach. Because adult Chinook salmon are naturally inclined to return to their upstream spawning tributaries, they are more likely to pass through the high-velocity section en route to their natal habitat. Conversely, steelhead may exhibit greater variability in migration routes due to downstream release points, potentially resulting in reduced apparent conversion. These differences highlight the importance of considering species-specific migration patterns, which are likely influenced by natal areas and release locations.
...
```{r rel-loc-tbl, message = F, tab.cap = "Summary of release location relative to the barrier reach for adult steelhead and sp/sum Chinook salmon observed at IPTDS sites in the South Fork Clearwater River."}
...
```{r cfs-p, fig.cap = "Conversion rates w/ 95% confidence intervals for sp/sum Chinook salmon and steelhead from SC3 to SC4 on the South Fork Clearwater River, by release location relative to the barrier reach."}
pass_by_cfs %>%
...
```{r elk-flow, fig.cap = "Estimated daily mean flow (cfs) at the Elk City gage during spring months, 2022 - 2025."}
sf_gage_int_df %>%
