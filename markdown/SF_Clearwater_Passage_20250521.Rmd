...
pass_by_rel_loc = sf_df %>%
  ...
  # join release site information from mrr_df
  left_join(mrr_df %>%
              select(siteCode,
                     type),
            by = c("rel_site" = "siteCode")) %>%
...
Finally, we evaluated the probability of an adult being detected at SC4 (given it arrived at SC3) depending on species, release location, and predicted flow (cfs) at the previous Elk City gage at the time of last detection. To do so, we fit a logistic regression model using species, release location, and estimated flow as covariates and probability of arriving at SC4 as the response variable. A fish that successfully arrived at SC4 was a "success" = 1 whereas a fish only seen at SC3 was "success" = 0. Figure \@ref(fig:lm) shows observed fish that arrived at SC4 (1) versus those only seen at SC3 (0) with the estimated flow at last detection (dots) as well as predicted probabilities of arriving at SC4 depending on species, release location, and flow.
...
lm_mod = glm(success ~ species + rel_loc + daily_cfs_elk, data = lm_df, family = binomial)
