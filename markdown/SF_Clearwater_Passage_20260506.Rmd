...
A section of the South Fork Clearwater River near milepost (MP) 28 has been identified as a potential migration barrier for adult steelhead, particularly during periods of elevated flows. Elevated water velocities in this reach are hypothesized to impede or occasionally prevent upstream migration, limiting access to spawning habitat in the upper mainstem and its tributaries. Previous research suggests that when river discharge exceeds approximately 1,000 cfs in the reach, at least one hydraulic feature produces velocities that surpass the burst swimming capacity of a 90 cm steelhead [@Timm2017; @Timm2018]. Earlier observations by Nez Perce Tribe (NPT) staff indicated that a velocity barrier may begin to form at flows above 600 cfs, based on readings from the former Elk City gage. Additionally, radio telemetry data collected from 2013 - 2017 [@Cleary2019] showed that relatively few steelhead spawned upstream of river kilometer (rkm) 71, which corresponds to the location of the suspected high-velocity reach (Figure \@ref(fig:loc-by-rkm)).
...
This report presents a preliminary assessment of adult steelhead passage at the suspected velocity barrier on the South Fork Clearwater River using detections of PIT-tagged fish. Key detection sites include SC3 (downstream of the barrier) and SC4 (upstream). The analysis includes adults PIT-tagged at Bonneville and Lower Granite dams and juveniles tagged at various locations, including within the South Fork Clearwater River. 

We focus on whether PIT-tagged adults detected at SC3 later appear at SC4, indicating successful upstream passage — or a lack thereof — across the high-velocity reach. We also examine how factors such as juvenile release location and river discharge relate to "conversion" rates. A conversion rate indicates the proportion of adults that successfully transition between key points during upstream migration and serves as a proxy for both survival and passage success. Although the primary focus is steelhead, adult spring/summer Chinook salmon PIT detections are included for comparison.

To support this evaluation, the number of PIT-tagged juvenile steelhead within Newsome Creek releases was increased starting in 2022 (Table \@ref(tab:juv-releases)). The first 1-ocean adults from these releases returned to spawn in spring 2024, with 2-ocean adults expected in 2025. Releases from 2023 will also begin returning as 1-ocean adults in spring 2025.

```{r juv-releases, tab.cap = "Number of PIT-tagged juveniles released (or anticipated) within Newsome Creek releases by year, including years that each release group will return to spawn."}
juv_rel_df = read_csv(here("data/derived_data/Tagging Detail 20250602.csv")) %>%
  clean_names() %>%
...
sf_df = comp_df %>%
  ...
  filter(!(t_rear_type == 'H' & rel_loc == 'Unknown'))
...
sf_df %>%
  group_by(species, spawn_year, rel_loc, t_rear_type) %>%
  summarise(n_tags = n(),
            .groups = "drop") %>%
...
lm_df = sf_df %>%
  ...
  # recode success to 0s and 1s
  mutate(success = ifelse(success, 1, 0)) %>%
  # focus on unknown and upstream release locations
  filter(!rel_loc == "Downstream")

# fit the logistic regression model
lm_mod_full = glm(success ~ species + rel_loc + daily_cfs_elk, data = lm_df, family = binomial(link = 'logit'))
lm_mod_red = glm(success ~ species + rel_loc, data = lm_df, family = binomial(link = 'logit'))
# LRT lm_mod_full vs lm_mod_red
test1 <- anova(lm_mod_red, lm_mod_full, test = "Chisq")

# fit only species model
lm_mod = glm(success ~ species, data = lm_df, family = binomial(link = 'logit'))
test2 <- anova(lm_mod, lm_mod_red, test = "Chisq")

# species only model fits the data best according to LRT
#pscl::pR2(lm_mod)

# summary of the model
lm_mod_summ = summary(lm_mod)

newdata <- data.frame(species = unique(lm_df$species))
pred <- predict(lm_mod, newdata = newdata, type = "response", se.fit = TRUE)
newdata$fit <- pred$fit
newdata$lwr <- pred$fit - 1.96 * pred$se.fit
newdata$upr <- pred$fit + 1.96 * pred$se.fit

# point estimates
ggplot(newdata, aes(x = species, y = fit)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = lwr, ymax = upr), width = 0.2) +
  scale_y_continuous(limits= c(0,1)) +
  labs(title = 'Predicted Probability of Passage',
       y = 'Predicted Probability with 95% CI',
       x = '') +
  theme_bw()

# model performance
# ggplot(lm_df, aes(x = predicted_prob, y = success)) +
#   geom_jitter(height = 0.05, width = 0.05, alpha = 0.4) +
#   geom_smooth(method = "glm", method.args = list(family = "binomial"), se = TRUE) +
#   labs(title = "Observed vs. Predicted Outcomes",
#        x = "Predicted Probability",
#        y = "Observed Outcome (0/1)") +
#   theme_minimal()


# these results are incorrect for the selected model
...
```{r fl-by-age, fig.cap = "Fork length by salwater age for PIT-tagged adult Chinook salmon and steelhead observed in the South Fork Clearwater River, spawn years 2022 - 2024."}
